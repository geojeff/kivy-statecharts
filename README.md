...
We still need mediating controllers as interfaces, but these should be small and operate via the bindings already fully functional in Kivy's property system, and the kv language system. By "small" here, we mean restricted in scope, not containing big logic blocks for application control. Keep that in the statechart system. The statechart system takes over *much* of the role played by coordinating controllers, offering the key advantages outlined above. The simple mediating controller role is covered by the existing Kivy properties system, where you create models as python classes, define properties in them that are bound to the user interface defined in kv files, and enjoy automatic updating. 
